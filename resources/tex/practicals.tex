...
In this problem, you are going to build a \textbf{Guess the Word} game. \\

Here is a brief explanation of the rules:
\begin{itemize}
  \item The game master (first player) thinks of a word, i.e., something simple like an animal, fruit, place, etc.
  \item The word is displayed as Xs where the number of Xs is equal to the length of the word.
  \item The guesser (second player) has to guess the word and is given a limited number of lives.
  \item The guesser provides a letter, and if the word contains the letter, the guess is correct. The game master will display the letter in all the correct positions of the word to be guessed.
  \item If the word does not contain the letter, the guesser loses a life.
  \item Once the guesser has run out of lives, the game is over.
  \item Once the guesser has guessed the word, display the number of guesses.
  \item Keep track of all guessed letters so that the guesser does not provide a letter more than once.
\end{itemize}
...
\begin{verbatim}
  ...
  The letter p is in the word. There are 2 occurrences.
  ...
  You guessed the word apple. It took you 5 guesses.
