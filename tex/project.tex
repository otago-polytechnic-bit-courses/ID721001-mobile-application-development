...
\begin{itemize}
	...
	\item An interactive quiz for each country.
	      \begin{itemize}
		      ...
		      \item At the end of each quiz, store the highest score in \textbf{SharedPreferences}.
	...
	\item \textbf{Spring Boot REST API} containing top-rated tourist attraction data.
	\begin{itemize}
		\item Each object must contain the name, country and location (latitude \& longitude).
		\item \item \textbf{Resource:} \footnotesize\href{https://kotlinlang.org/docs/jvm-spring-boot-restful.html}{https://kotlinlang.org/docs/jvm-spring-boot-restful.html}
	\end{itemize} 
	\item Google Map displaying top-rated tourist attraction data.
	      \begin{itemize}
		      \item Use \textbf{Retrofit} \& the \textbf{Spring Boot REST API} to display a marker for each object on a \textbf{Google Map}.
		      \item The marker's information window must display the attraction name \& coordinates.
	...
	\item \textbf{BottomNavigationView} which navigates the user to the appropriate activities. For example, a menu icon for translation support, text to speech support, etc.
	      ...
	      \end{itemize}
	\item Display the application's privacy policy in a \textbf{WebView}.
	      \begin{itemize}
	      ...
	      \end{itemize}
	\item Visually attractive user-interface with a coherent graphical theme \& style using \textbf{Material Design}.
	\item Application is published to \textbf{Google Play Store}.
	      \begin{itemize}
		      \item To published to \textbf{Google Play Store}, you will need a \textbf{Google Play Console} account. The account's credentials are available in the \textbf{Microsoft Teams} course channel, under the \textbf{Files} tab. The account will be available to all learners in the course. Do not disable any applications published on this account.
		      \item When you create the application, name the package appropriately. For example, \\ \textbf{op.mobile.app.dev.$<$username$>$.travelling}. \textbf{Note:} replace \textbf{username} with your \textbf{Otago Polytechnic} username.
		      \item \textbf{Resources:}
		            \begin{itemize}
		            ...
		            \end{itemize}
	      \end{itemize}
	\item Ability to download the application from \textbf{Google Play Store} on to multiple mobile devices. The mobile devices used to download your application will be:
	      \begin{itemize}
		      \item Pixel 2 - 5.0"
		      \item Pixel XL - 5.5"
		      \item Pixel 3a XL - 6.0"
	      \end{itemize}
	\item At least \textbf{15} user-interface tests which verify that the UI of your application is functioning correctly.
\end{itemize}

\subsection*{Code Elegance - Learning Outcomes 1, 4 (40\%)}
\begin{itemize}
...
\subsection*{Documentation \& Git/GitHub Usage - Learning Outcomes 3, 4 (20\%)}
\begin{itemize}
	\item Provide the following in the repository \textbf{README} file:
	      \begin{itemize}
		      ...
		      \item \textbf{Resources:}
		            ...
		            \end{itemize}
		      \item \textbf{Spring Boot REST API} endpoints.
	      \end{itemize}
	\item At least \textbf{10} feature branches excluding the \textbf{main} branch.
	      ...
	      \end{itemize}
	\item Setup continuous integration using \textbf{GitHub Actions}.
	      \begin{itemize}
		      \item The reviewer must not be able to merge the pull request until \textbf{all} UI tests pass.
	      ...
	      \end{itemize}
	\item Commit messages must reflect the context of each functional requirement change. Do not rewrite your \textbf{Git} history. It is important that the course lecturer can see how you worked on your assessment over time.
	      \begin{itemize}
