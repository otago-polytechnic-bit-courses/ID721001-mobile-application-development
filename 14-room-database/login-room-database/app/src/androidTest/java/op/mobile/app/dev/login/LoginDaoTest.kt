...
class LoginDaoTest {
    ...
    @Test
    @Throws(IOException::class)
    fun insertLoginDetailAndGetUsername() = runBlocking {
        val login = Login("john.doe", "P@ssw0rd123")
        dao.insert(login)
        val allLoginDetails = dao.getAll().first()
        assertEquals(allLoginDetails[0].username, login.username)
    }
}